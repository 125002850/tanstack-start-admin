...
describe('data-table-column-resize-storage', () => {
  ...
  describe('loadColumnSizing', () => {
    ...
    it('filters out zero or negative sizing values', () => {
      localStorage.setItem(
        STORAGE_KEY,
        buildCache({ name: 0, email: -10, role: 100 }),
      )
    ...
    it('different tableIds are isolated', () => {
      localStorage.setItem(
        'data-table:table-a:column-sizing',
        buildCache({ col: 100 }),
      )
      localStorage.setItem(
        'data-table:table-b:column-sizing',
        buildCache({ col: 200 }),
      )
  ...
  describe('saveColumnSizing', () => {
    ...
    it('different tableIds write to different keys', () => {
      ...
      expect(JSON.parse(localStorage.getItem('data-table:table-a:column-sizing')!).sizing).toEqual({ a: 1 })
      expect(JSON.parse(localStorage.getItem('data-table:table-b:column-sizing')!).sizing).toEqual({ b: 2 })
    })
  })
  ...
  describe('clearColumnSizing', () => {
    ...
    it('only clears the targeted tableId', () => {
      ...
      expect(
        localStorage.getItem('data-table:table-a:column-sizing'),
      ).toBeNull()
      expect(
        localStorage.getItem('data-table:table-b:column-sizing'),
      ).not.toBeNull()
    })
  })

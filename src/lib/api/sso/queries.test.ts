...
const mockSession = {
  ...
  setLoginUserId: vi.fn<(userId: string) => void>(),
...
vi.mock('./session', () => ({
  ...
  setLoginUserId: (userId: string) => mockSession.setLoginUserId(userId),
...
vi.mock('../clients/service/generated/runtime', () => ({
  transportProfile: {
    get basePath() {
      return mockRuntime.basePath;
    }
  }
}));
...
describe('login info query', () => {
  ...
  it('writes logoutUrl to session on success', async () => {
    ...
    expect(mockSession.setLoginUserId).toHaveBeenCalledWith('1');
  });

  it('prefixes login info requests when basePath is non-root', async () => {
    mockRuntime.basePath = '/admin-api';
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          rspCode: '200',
  ...
  it('does not set logoutUrl on failure', async () => {
    ...
    expect(mockSession.setLoginUserId).not.toHaveBeenCalled();
  });
  ...
  it('throws login forbidden error and caches logoutUrl on HTTP 403', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          code: 403,
    ...
    const options = await importQueryOptions();
    const { isLoginForbiddenError } = await import('./errors');
    ...
    expect(isLoginForbiddenError(caught)).toBe(true);
    expect((caught as Error).message).toBe('没有访问该资源的权限');
    expect((caught as Error & { logoutUrl?: string }).logoutUrl).toBe('https://sso/logout');
    expect(mockSession.setLogoutUrl).toHaveBeenCalledWith('https://sso/logout');
    expect(mockSession.setLoginUserId).not.toHaveBeenCalled();
  });

  it('treats business code 403 as login forbidden even when HTTP status is 200', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          rspCode: '403',
    ...
    expect(isLoginForbiddenError(caught)).toBe(true);
    expect((caught as Error & { loginUrl?: string }).loginUrl).toBe('https://sso/login');
    expect(mockSession.setLogoutUrl).toHaveBeenCalledWith('https://sso/logout');
    expect(mockSession.setLoginUserId).not.toHaveBeenCalled();
  });
});

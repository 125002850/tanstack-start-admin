...
it('keeps raw browser requests inside the SSO bootstrap boundary', () => {
  const violations = collectFiles(
    ...
    .filter((path) => {
      ...
      return (
        ...
        relativePath !== 'src/lib/api/sso/bootstrap.ts' &&
        (/\bfetch\s*\(|new\s+XMLHttpRequest\b/.test(readFileSync(path, 'utf8')) ||
          (relativePath !== 'src/lib/api/sso/queries.ts' &&
            /\bbootstrapRequest\b/.test(readFileSync(path, 'utf8'))))
      );
    })
    .map(toProjectPath);
  expect(violations).toEqual([]);

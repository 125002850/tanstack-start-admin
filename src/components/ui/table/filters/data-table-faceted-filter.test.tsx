...
describe('DataTableFacetedFilter tree options', () => {
  ...
  it('widens the popover when hierarchical options are present', () => {
    renderTreeFilter();

    expect(document.querySelector('.data-table-filter-popover')).toHaveClass('w-72');
  });

  it('renders a visual hierarchy marker for child options', () => {
    renderTreeFilter();

    const childOption = screen.getByRole('button', { name: /研发部/ });
    expect(childOption.querySelector('[aria-hidden="true"]')).toHaveTextContent('└');
  });
});

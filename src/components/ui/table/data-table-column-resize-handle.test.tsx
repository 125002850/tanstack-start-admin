...
vi.mock('@tanstack/react-virtual', () => ({
  useVirtualizer: ({ count, enabled, estimateSize }: {
    count: number; enabled?: boolean; estimateSize: () => number
  }) => {
...
vi.mock('@/components/ui/scroll-area', () => ({
  ScrollArea: ({ children, viewportRef, viewportProps }: {
...
describe('DataTableColumnResizeHandle overlay lifecycle', () => {
  ...
  it('creates overlay in overlay root on mousedown', () => {
    ...
    const overlay = children.find((el) =>
      (el as HTMLElement).style.position === 'absolute' && (el as HTMLElement).style.pointerEvents === 'none'
    )
  ...
  it('overlay is removed on mouseup', () => {
    ...
    const overlayDuring = childrenDuring.find((el) =>
      (el as HTMLElement).style.position === 'absolute' && (el as HTMLElement).style.pointerEvents === 'none'
    )
    ...
    const overlayAfter = childrenAfter.find((el) =>
      (el as HTMLElement).style.position === 'absolute' && (el as HTMLElement).style.pointerEvents === 'none'
    )

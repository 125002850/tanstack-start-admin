...
test('task0: baseline — perPage=2000 profiling, column width stability, sticky header @workspace-v2-rollback', async ({
  page
}) => {
  ...
  const viewportTop = await viewport.evaluate((el) => Math.round(el.getBoundingClientRect().top));
  ...
  const virtualEnabled = await viewport.evaluate((el) => el.getAttribute('data-virtual-enabled'));
